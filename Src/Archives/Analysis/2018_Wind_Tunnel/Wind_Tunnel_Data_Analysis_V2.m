...
for i = ID_Angles_Attaque
    ...
    % Averaged density:
    density = mean(Current_Exp(Data_pos,Pos_Density)); 
...
%         Fd_model = [Fd_model drag(Rocket,AA,V_model(i),Nu,343)*0.5*density*Rocket.maxCrossSectionArea*V_model(i)^2];
%     end
...
%         Fn_model = [Fn_model CNa*AA*0.5*density*Rocket.maxCrossSectionArea*V_model(i)^2];
%     end
...
%     legend show;xlabel('Wind Speed [m/s]');ylabel('F_N [N]');
    ...
    for i = 1:length(V_model)
        ...
        Fy_model = [Fy_model (CNa*AA+sin(AA)*drag(Rocket,AA,V_model(i),Nu,343))*0.5*density*Rocket.maxCrossSectionArea*V_model(i)^2];
    end
...
for i = ID_Aerofreins
    ...
    V_model = linspace(20,80,30);Fd_model = [];
    for i = 1:length(V_model)
        Fd_model = [Fd_model drag(Rocket,0,V_model(i),Nu,343)*0.5*density*Rocket.maxCrossSectionArea*V_model(i)^2];
    end
