...
dm = Rocket.maxDiameter; % maximum rocket diameter 
Sm = Rocket.maxCrossSectionArea; % maximum cross-sectional body area
% c = Rocket.meanFinChord; % fin cord
% SE = Rocket.exposedFinArea; % Exposed planform fin area
% df = Rocket.finBodyDiameter; % body diameter at middle of fin station
% SF = Rocket.virtualFinArea; % Virtual fin planform area
...
% 2.1 Body 
Rl = Rocket.stagePositions(end)*Uinf/nu; 
Rl_crit = 5e5;
% % 2.2 Fins
% Rc = c*Uinf/nu; 
% Rc_crit = 5.14e6;
...
SsSm = 2/dm^2*sum((Rocket.stageDiameters(2:end-1)+ Rocket.stageDiameters(3:end)).*...
    ...
    (Rocket.stagePositions(3:end)-Rocket.stagePositions(2:end-1))).^2)); 
if strcmp(Rocket.coneMode, 'on')
    SsSm = SsSm + 2.67*Rocket.stagePositions(2)/dm;
end

if Rocket.stagePositions(2)/dm < 1.5
    display('WARNING: In drag coefficient calculation, ogive cone ratio is out of bounds. Drag estimation cannot be trusted.');
end

% 4.2 Body drag
% 4.2.1 (eq 161, p 431)
CDf_B = (1+60/(Rocket.stagePositions(end)/dm)^3+0.0025*(Rocket.stagePositions(end)/dm))*SsSm; % partially calculated body drag
if Rl < Rl_crit
...
CDb = 0.029*(Rocket.stageDiameters(end)/dm)^3/sqrt(CDf_B);
% 4.2.3 Body drag at 0?? AoA (eq 160, p 431)
CD0_B = CDf_B +CDb; 

% 4.3 Fin drag
% 4.3.1 Fin drag at 0?? AoA (eq 159, p 433)
% CD0_F = 2*(1+2*Rocket.finThickness/c)*Rocket.numFins*SF/Sm; 
% if Rc < Rc_crit
...
% coefficient.
CD_l = Rocket.numLaunchLugs*5.75*Rocket.lugSurfaceArea/Sm;
...
etak = interp1(etatab(1,:),etatab(2,:),Rocket.stagePositions(end)/dm,'linear','extrap');
deltak=interp1(deltaktab(1,:),deltaktab(2,:),Rocket.stagePositions(end)/dm,'linear','extrap');
% 5.1.2 Compute body drag at angle of attack alpha
% 5.1.2.1 x1 as defined by explanations of (eq 140, p 404)
x1 = Rocket.stagePositions(find(diff(Rocket.stageDiameters)==0, 1, 'first'));
...
tmp_diameters = [interp1(Rocket.stagePositions, Rocket.diameters, x0, 'linear'), Rocket.diameters(Rocket.stagePositions>x0)];
...
% Rs = df/(2*Rocket.finSpan+df); % Total fin span ratio
...
% CD_t_fin = 1.42*fin_efficiency(Rocket.numFins);
% CD_t_body = 0.56;

% CD_t = (SE*CD_t_fin+CD_t_body*dm*(Rocket.stagePositions(end)-Rocket.stagePositions(2)))/Sm;
CD_t = 0;
