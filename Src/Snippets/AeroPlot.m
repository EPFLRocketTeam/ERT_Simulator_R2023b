...
if readRocketFile
   try
       ...
       fprintf('  Center of mass (empty): %.3f m\n', Rocket.emptyCenterOfMass);
       fprintf('  Reference Area: %.4f m²\n', Rocket.maxCrossSectionArea);
   catch ME
...
try
    ...
    error('AeroPlot:CalculationError', 'Failed during aerodynamic calculations: %s', ME.message);
end
...
try
    ...
    error('AeroPlot:PlotError', 'Failed during plot generation: %s', ME.message);
end
